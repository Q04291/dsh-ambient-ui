...
export function TrailAnimation(props) {
    ...
    const countsRef = useRef({ think: 0, tool: 0, output: 0 });
    ...
    useEffect(() => {
        ...
        const spawned = [];
        for (const kind of ['think', 'tool', 'output']) {
            for (let i = prev[kind]; i < counts[kind]; i += 1)
                spawned.push(kind);
        }
        ...
        if (snapshot.running && spawned.length === 0) {
            ...
            if (counts.think > 0 && blocks.some((b) => b.kind === 'reasoning'))
                spawned.push('think');
            else if (counts.output > 0 && blocks.some((b) => b.kind === 'text'))
                spawned.push('output');
        }
