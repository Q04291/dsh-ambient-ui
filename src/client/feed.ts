...
/** One assistant stream block the trail classifies. */
export interface AmbientFeedBlock {
  kind: 'reasoning' | 'text' | 'tool-call' | (string & {})
}

/** The chat-target snapshot surface the trail reads (the `legacy` projection). */
export interface AmbientChatSnapshot {
  legacy: {
    /** In-progress assistant output, absent while no assistant turn is streaming. */
    partial: { blocks: readonly AmbientFeedBlock[] } | null
    /** Live (running) tool calls. */
    runningCalls: readonly { callId: string }[]
  }
}

declare module '@deepseek-ai/dsh-client-ui-slots' {
  interface SessionStandardProps {
    /** Chat target feed (contributed by ui-chat when the Chat target is mounted). */
    useChat: <T>(selector: (snapshot: AmbientChatSnapshot) => T) => T
  }
}
